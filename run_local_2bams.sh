...
# run per BAM file
p=clipped_reads
python $p.py -b "$BAMSV" -c "${SEQ_IDS_CSV}" -o $p.json.gz -p . -l $p.log
...
p=split_reads
python $p.py -b "$BAMSV" -c "${SEQ_IDS_CSV}" -o $p.json.gz -ob $p.bedpe.gz -p . -l $p.log


# write channels into *.json.gz and *.npy.gz files
for s in "${SEQ_IDS[@]}"; do  # per chromosome
  p=clipped_read_distance
  python $p.py -b "$BAMSV" -c $s -o $p.json.gz -p . -l $p.log
