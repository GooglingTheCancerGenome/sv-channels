...
def get_candidate_positions():
    ...
    sampleName = 'T1'
    outFile = os.path.join(channel_dir, sampleName,
                           'candidate_positions.json.gz')
    ...
    data_id = [
        '_'.join((chr1, str(pos1), chr2, str(pos2)))
        for chr1, pos1, chr2, pos2 in candpos_list
        if chr1 == chr2 and chr1 == '17'
    ] * 23
...
def train(sampleName):
    ...
    partition = {
        'train': data_id[:train_index],
        'validation': data_id[train_index:]
    }
    ...
    labels = {
        k: i
        for k, i in zip(data_id, np.random.randint(2, size=len(data_id)))
    }

    # Generators
    training_generator = DataGenerator(chr_array, partition['train'], labels,
                                       **params)
    validation_generator = DataGenerator(chr_array, partition['validation'],
                                         labels, **params)
    ...
    model.fit_generator(
        ...
        epochs=2,
        workers=
        batch_size,  # I don't see multi workers can have any performance benefit without multi threading
