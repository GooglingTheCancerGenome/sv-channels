...
p=clipped_reads
cmd="python $p.py -b '$BAMSV' -c '${SEQ_IDS_CSV}' -o $p.json.gz -p . -l $p.log"
...
p=split_reads
cmd="python $p.py -b '$BAMSV' -c '${SEQ_IDS_CSV}' -o $p.json.gz -ob $p.bedpe.gz \
  -p . -l $p.log"
JOB_ID=$(submit $p all "$cmd")
JOBS+=($JOB_ID)
...
for s in "${SEQ_IDS[@]}"; do  # per chromosome
  ...
  cmd="python $p.py -b '$BAMSV' -c $s -o $p.json.gz -p . -l $p.log"
