...
gr_tmp <- gr[[sample]][['manta']]
print(length(gr_tmp))
gr_tmp <- gr_tmp[overlapsAny(gr_tmp, confidence_regions_gr) &
     overlapsAny(partner(gr_tmp), confidence_regions_gr), ]
print(length(gr_tmp))
...
truth_set_file[['CHM1_CHM13']] <-
  '/Users/lsantuari/Documents/Data/germline/CHM/Huddleston2016/structural_variants/CHM1_CHM13_SVs.annotated.vcf.gz'
...
#for(sv_caller in c('manta'))
{
  ...
  # vcf_file <- file.path('/Users/lsantuari/Documents/Data/germline',sample,'SV/Filtered',paste(sv_caller,'.vcf',sep=''))
...
for(sv_caller in sv_caller_list)
{
gr[[sample]][[sv_caller]]$caller <- sv_caller
}

# svgr <- c(gr[[sample]][['gridss']],
#           gr[[sample]][['manta']],
#           gr[[sample]][['lumpy']],
#           gr[[sample]][['delly']])

# svgr <- gr[[sample]][['manta']]
