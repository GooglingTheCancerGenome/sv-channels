...
def get_split_read_positions(ibam, chr_list, min_mapq, min_sr_support, outFile, outBedpe):
            ...
            if read.has_tag('SA'):
                ...
                if sa_entry is not None:
                    ...
                    if chr_SA in chr_list:
                        ...
                        split_read_distance[read.reference_name][clipped_ch][
                            clipped_pos].append(dist)
    ...
    for chrom in chr_list:
        for k in ['right', 'left']:
            logging.info('Number of unique ' + k +
                         ' split read positions on Chr{}: {}'.format(
                             chrom,
                             len([
    ...
    for k in split_pos_coord.keys():
        logging.info('Number of unique pair of split read positions ' + k +
                     ': %d' % len(split_pos_coord[k]))
    ...
    for k in total_reads_coord_min_support.keys():
        logging.info('Number of total pairs of ' + k +
                     ' positions with min support: %d' %
                     len(total_reads_coord_min_support[k]))
