...
def read_svcaller_vcf(invcf, svcaller):
    ...
    for rec in vcf_in.fetch():
        ...
        elif var.start > var.end:

            chrom1 = var.chrom2
            pos1_start = var.end + var.ciend[0]
            pos1_end = var.end + var.ciend[1] + 1

            chrom2 = var.chrom
            pos2_start = var.start + var.cipos[0]
            pos2_end = var.start + var.cipos[1] + 1
            svtype = var.svtype

        if svtype == "DEL":
            sv = (
                chrom1, pos1_start, var.start, pos1_end,
                chrom2, pos2_start, var.end, pos2_end,
                svtype
            )
            if sv not in sv_set:
                sv_list.append(sv)
    ...
    logging.info('{} SVs in svcaller {}'.format(len(sv_list), svcaller))
...
def overlap(sv_list, cpos_list, win_hlen, ground_truth, outDir):
    ...
    def search_tree_with_cpos(cpos, trees_start, trees_end):
        ...
        for i, p in enumerate(cpos, start=1):
            ...
            # if pos1c - win_hlen <= pos1l and pos1r <= pos1c + win_hlen and \
            #         pos2c - win_hlen <= pos2l and pos2r <= pos2c + win_hlen:
            ...
            if pos1 - win_hlen <= lu_start_elem_start and lu_start_elem_end <= pos1 + win_hlen and \
                ...
                logging.info(
                    'Chr1:{}\tpos1:{}-{}-{}\tChr2:{}\tpos2:{}-{}-{}'.format(
                        chrom1, pos1 - win_hlen, pos1, pos1 + win_hlen,
                        chrom2, pos2 - win_hlen, pos2, pos2 + win_hlen
                    )
                )
                logging.info(
...
# Get labels
def get_labels(ibam, sampleName, svcaller, win_len, ground_truth, outFile, outDir):
    ...
    invcf = os.path.join('/Users/lsantuari/Documents/Data/germline/',
                         sampleName, 'SV/Filtered', svcaller+'.vcf')
    cpos_list = read_svcaller_vcf(invcf, svcaller)
    # for sventry in cpos_list:
    #     print(sventry)

    # Keep only positions that can be used to create windows
    chr_len_dict = get_chr_len_dict(ibam)
    cpos_list = [(chrom1, pos1c, chrom2, pos2c) for chrom1, pos1l, pos1c, pos1r,
                                                    chrom2, pos2l, pos2c, pos2r, svtype in cpos_list
                 if chrom1 in chrlist and chrom2 in chrlist and
                 win_hlen <= pos1l and  pos1r <= chr_len_dict[chrom1] - win_hlen and
                 win_hlen <= pos2l and pos2r <= chr_len_dict[chrom2] - win_hlen]
...
def main():
    ...
    parser.add_argument('-s', '--sample', type=str, default='NA24385',
                        help="Specify sample")
    parser.add_argument('-svc', '--svcaller', type=str, default='gridss',
                        help="Specify svcaller for candidate positions")
    parser.add_argument('-w', '--window', type=str, default=1000,
    ...
    output_dir = os.path.join(args.outputpath, args.sample, cmd_name, args.svcaller)
