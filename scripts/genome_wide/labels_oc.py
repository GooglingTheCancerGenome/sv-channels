...
def read_bpi(sampleName, sv_caller):
    '''
    This function parses the entries of the nanosv VCF file into SVRecord_nanosv objects and
    returns them in a list
    :param sampleName: str, name of the sample to consider
...
def read_sample_list():
    if not HPC_MODE:
        channel_dir = '/Users/lsantuari/Documents/Data/HPC/DeepSV/GroundTruth/OC/'
    else:
        channel_dir = '/hpc/cog_bioinf/ridder/users/lsantuari/Git/DeepSV_runs/' + \
                      '041018/CNN/scripts/genome_wide/SGE_scripts'

    with open(os.path.join(channel_dir, 'OC_comparisons.txt'), 'r') as f:
        sample_list = set(['--'.join(line.strip().split('\t')) for line in f])
    f.closed
...
def main():
    ...
    #sample_list = read_sample_list()
    #print(sample_list)

    #for sampleName in sample_list:

    get_labels(args.sample)
