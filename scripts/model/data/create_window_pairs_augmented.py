...
def get_windows(sampleName, outDir, win, cmd_name, sv_caller, mode, npz_mode):
    ...
    for labs_name, labs in labels_set.items():
        ...
        bcolz_array = bcolz.carray(
            bcolz.zeros(shape=(0, int(win) * 2 + padding_len, n_channels),
                        dtype=np.float32),
            mode='w',
        ...
        for k, val in labs.items():
            ...
            for p1 in np.arange(pos1 - win_hlen, pos1 + win_hlen):
                ...
                for p2 in np.arange(pos2 - win_hlen, pos2 + win_hlen):
                    ...
                    dask_array = list()
                    d = chr_array[chr1][p1 - win_hlen:p1 + win_hlen, :]
                    dask_array.append(d)
                    dask_array.append(padding)
                    d = chr_array[chr2][p2 - win_hlen:p2 + win_hlen, :]
                    dask_array.append(d)
