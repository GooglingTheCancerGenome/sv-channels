...
def parse_cl_args(in_args):
    ...
    parser.add_argument("-i", "--input_bedpe", type=str,
                        default=os.path.join(working_dir, 'simulated-data-results/coverage/cov30/hmz-sv/sv-channels.split_reads.bedpe'),
    ...
    parser.add_argument("-b", "--twobit", type=str,
                        default=os.path.join(working_dir, 'helpers/aux_files/hs37d5.2bit'),
                        help="Path to 2bit file")
    parser.add_argument("-o", "--output_vcf", type=str,
                        default=os.path.join(working_dir, 'simulated-data-results/coverage/cov30/hmz-sv/sv-channels.split_reads.vcf'),
                        #default=os.path.join(working_dir,
...
def convert_bedpe(input_bedpe_fn, output_vcf_fn, sample_name, genome):
    ...
    for k in sorted(genome.keys()):
        vcf_header_elems.append("##contig=<ID={},length={}>".format(k, len(genome[k])))

    vcf_header_elems.append("#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t{}".format(sample_name))
    ...
    for sv_calls in input_bedpe.itertuples(index=False, name='Pandas'):
        ...
        svtype = svtype.split('_')[1] if svtype not in ['DEL', 'INS', 'INV', 'DUP', 'CTX', 'TRA'] else svtype
        ...
        if svtype not in ['TRA', 'CTX']:
            ...
            if svtype == 'INS':
                info_field = "SVTYPE={};END={};SVLEN={}".format(svtype, str(sv_calls[1]+1), svlen)
            else:
                info_field = "SVTYPE={};END={};SVLEN={}".format(svtype, end, svlen)
            ...
            bp1_alt_1 = bp1_ref_fw + brkt_bw + chrom2 + ':' + str(bp2_pos) + brkt_bw
            bp1_alt_2 = brkt_fw + chrom2 + ':' + str(bp2_pos) + brkt_fw + bp1_ref_rv

            # bp1_1
            bp1_1_info_field = "SVTYPE={};MATEID={}".format('BND', get_identifier(calls_index, 4))
            ...
            # bp1_2
            bp1_2_info_field = "SVTYPE={};MATEID={}".format('BND', get_identifier(calls_index, 3))
            ...
            bp2_alt_1 = bp2_ref_rv + brkt_bw + chrom + ':' + str(bp1_pos) + brkt_bw
            bp2_alt_2 = brkt_fw + chrom + ':' + str(bp1_pos) + brkt_fw + bp1_ref_fw

            # bp2_1
            bp2_1_info_field = "SVTYPE={};MATEID={}".format('BND', get_identifier(calls_index, 2))
            ...
            # bp2_2
            bp2_2_info_field = "SVTYPE={};MATEID={}".format('BND', get_identifier(calls_index, 1))
