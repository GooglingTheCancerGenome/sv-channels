...
wd <-
  ...
  '/Users/lsantuari/Documents/Processed/Benchmark_GPU/130918/run2/'
...
for (sample_name in c('NA12878', 'PATIENT1', 'PATIENT2'))
{
  ...
  res_long_DF <- summarySE(data = res_long, measurevar = 'value', groupvars = c('evidence', 'label', 'variable'))
  ...
  p <-
    ggplot(res_long_DF, aes(x = variable, y = value, fill = evidence)) +
    geom_bar(stat = "identity", position = position_dodge())
  p <- p + scale_fill_brewer(palette = "Paired") + theme_minimal()
