...
def main():
    ...
    for coord in start_end_SV_DEL: #[0:100] #changed 'i' to 'coord'
        ...
        for element1 in Gclass.keys(): #should be 2 elements
            ...
            all_reads_in_window_file_name_G = all_reads_in_window(Gclass[element1],chromosome_number,left,right,element1,0)
        ...
        for element2 in Sclass.keys():
            all_reads_in_window_file_name_S = all_reads_in_window(Sclass[element2],chromosome_number,left,right,element2,0)
            number_of_reads_in_window_total_S = number_of_reads_in_window_compute(all_reads_in_window_file_name_S)
            matrix_str_updated_S, matrix_int_left_updated_S, matrix_int_right_updated_S = matrix_read_updater_for_str_int(all_reads_in_window_file_name_S,coord,window_to_each_side,number_of_reads_in_window_total_S,full_window_length)
            vstack_12_channels_S = channels_12_vstacker(matrix_str_updated_S,matrix_int_left_updated_S,matrix_int_right_updated_S,current_genome_reference)
        ...
        vstack_12_channel_pairer_plus_GC_chanel_S = vstack_12_channel_pairer_plus_GC_chanel_fcn(vstack_12_channels_pair_Sclass_list[0],vstack_12_channels_pair_Sclass_list[1],GC_content)
