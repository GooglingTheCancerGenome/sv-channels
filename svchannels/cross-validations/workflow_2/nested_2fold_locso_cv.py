...
def train_test_val_split_by_sample_and_chrom_list(X, y, test_sample, test_chrom_list, val_chrom):
    idx_train = np.where(np.logical_and(y[:, 0] != test_sample,
                                        np.logical_and.reduce([y[:, 1] != test_chrom
                                                               for test_chrom in test_chrom_list]),
                                        y[:, 1] != val_chrom))[0]
    idx_test = np.where(np.logical_and(y[:, 0] == test_sample,
                                       np.logical_or.reduce([y[:, 1] == test_chrom
                                                             for test_chrom in test_chrom_list]),
                                       ))[0]
...
def train(input_args):
    ...
    for outer_i, outer_c_list in enumerate([first_list, second_list]):
        ...
        for outer_c in outer_c_list:
            ...
            assert outer_c in np.unique(outer_y_test[:, 1]), "Test chrom {} not in test set: {}".format(
